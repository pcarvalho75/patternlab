...
namespace PatternTools.CSML
{
    /// <summary>
    /// Code from http://www.codeproject.com/Articles/19032/C-Matrix-Library
    /// </summary>
    /// 
    [Serializable]
    public class Matrix
    {
        ...
        //These methods were included by Paulo C Carvalho

        /// <summary>
        /// Retrieves the real numbers of a row
        /// </summary>
        /// <param name="rowNo"></param>
        /// <returns></returns>
        public List<double> GetRow(int rowNo)
        {
            ...
            for (int i = 0; i < Values.Count; i++)
            {
                ...
                foreach (Complex j in (ArrayList)Values[i])
                {
                    l.Add(j.Re);
                }
            }
        ...
        public Matrix(List<List<double>> valuesL)
        {
            //Convert this to a 2D array and call the other constructor
            int rows = valuesL[0].Count;
            int cols = valuesL.Count;
        ...
        /// <summary>
        /// Returns the shortest path between two given vertices pn and j as
        /// int array.
        /// </summary>
        /// <param name="P">Path matrix as returned from Floyd().</param>
        /// <param name="pn">One-based index of start vertex.</param>
        ...
        /// </summary>
        /// <param name="adjacence_matrix">A[pn,j] = 0 or +infty, if there is no edge from pn to j; any non-zero value otherwise.</param>
        ...
        public static Matrix DFS(Matrix adjacence_matrix, int root)
        {
            ...
            if (root < 1 || root > n)
                throw new ArgumentException("Root must be a vertex of the graph, e.pn. in {1, ..., n}.");
        ...
        /// </summary>
        /// <param name="pn">One-based index at which to delete.</param>
        ...
        /// </summary>
        /// <param name="pn">One-based index at which to extract.</param>
        ...
        /// <param name="pn">One-based index at which to insert</param>
        ...
        /// <summary>
        /// Inserts a sub matrix M at row pn and column j.
        /// </summary>
        /// <param name="pn">One-based row number to insert.</param>
        ...
        /// Be careful, not working very well
        /// </summary>
        ...
        /// <summary>
        /// Retrieves row with one-based index pn.
        /// </summary>
        /// <param name="pn"></param>
        /// <returns>pn-th row...</returns>
        public Matrix Row(int i)
        {
            if (i <= 0 || i > rowCount)
                throw new ArgumentException("Index exceed matrix dimension.");

            //return (new Matrix((Complex[])((ArrayList)Values[pn - 1]).ToArray(typeof(Complex)))).Transpose();
        ...
        /// <summary>
        /// Swaps each matrix entry A[pn, j] with A[j, pn].
        /// </summary>
        ...
        /// <summary>
        /// Replaces each matrix entry posSignalToAccumulate = x + iy with x - iy.
        /// </summary>
        ...
        public Matrix LUSafe()
        {
            ...
            for (int j = 1; j <= n; j++)
            {
                ...
                // find index m with |this[m,j]| >= |this[pn,j]| for all pn in {j,...,n}
                if (j < n)
                {
                ...
                for (int i = j + 1; i <= n; i++)
                {
                    if (this[j, j] == 0)
                    {
                        Console.WriteLine("Warning: Matrix close to singular.");
        ...
        /// by a_{11} = h_{11}^2, a_{ii} = h_{ii}^2 + \sum_{chargeHypothesis=1}^{pn-1}h_{ik}^2 (pn = 2..n).
        ...
        /// <summary>
        /// Product G(pn,chargeHypothesis,theta)'*this. Internal use for QRGivens.
        /// </summary>
        /// <param name="isotopicStep"></param>
        /// <param name="s"></param>
        /// <param name="pn"></param>
        /// <param name="chargeHypothesis"></param>
        /// <param name="j1"></param>
        /// <param name="j2"></param>
        /// <returns></returns>
        ...
        /// <summary>
        /// Product M*G(pn,chargeHypothesis,theta). Internal use for QRGivens.
        /// </summary>
        /// <param name="isotopicStep"></param>
        /// <param name="s"></param>
        /// <param name="j1"></param>
        ...
        /// if *this is not a permutation matrix (e.pn. a permutation of Id),
        /// garbage is returned.
        /// </summary>
        /// <returns></returns>
        ...
        /// <returns></returns>
        /// <remarks>If p not in {pn, +infty}, *this must be a vector.</remarks>
        ...
        /// </summary>
        /// <param name="pn">One-based index of the col to consider.</param>
        /// <returns></returns>
        ...
        /// </summary>
        /// <param name="pn">One-based index of the row to consider.</param>
        /// <returns></returns>
        ...
        public DefinitenessType Definiteness()
        {
            ...
            Matrix xk; // to buffer this.Column(chargeHypothesis)
            Matrix buf;
            ...
            // e.g. A pos. def. <=> A > 0 <=> y[pn]'Ay[pn] > 0 for all pn (same for neg. def., ...)
        ...
        /// <summary>
        /// Checks if matrix is involutary, e.pn. if A*A = id.
        /// </summary>
        ...
        //---------------------------------------------------
        public static Matrix KernelGaussianDot (Matrix A, Matrix B, double sigma)
        {
            if (A.ColumnCount != B.RowCount)
                throw new ArgumentException("Inner matrix dimensions must agree.");
            ...
            for (int i = 1; i <= A.RowCount; i++)
            {
                for (int j = 1; j <= B.ColumnCount; j++)
                {
                    Complex r = new Complex(KernelProduct(A.Row(i), B.Column(j), sigma).Re);
                    C[i, j] = r;
                }
            }
        ...
        public static Complex KernelProduct(Matrix v, Matrix w, double sigma)
        {
            int m = v.VectorLength();
            int n = w.VectorLength();
            ...
            for (int i = 1; i <= m; i++)
            {
                double d = v[i].Re - w[i].Re;
                norm += Math.Pow(d, 2);   
            }

            //double beta = 2.0 * sigma * sigma;
            //return new Complex(System.Math.Exp(-norm / beta));
            return (new Complex(norm));
        }
        ...
        /// <summary>
        /// Access the component in row pn, column j of a non-empty matrix.
        /// </summary>
        /// <param name="pn">One-based row index.</param>
        ...
        public virtual Complex this[int i, int j]
        {
            set
            {
                ...
                if (i > rowCount)
                {
                    // dynamically add pn-Rows new rows...
            ...
            get
            {
                if (i > 0 && i <= rowCount && j > 0 && j <= columnCount)
                {
                    ...
                    //try
                    //{
                    //    buf = (Complex)(((ArrayList)Values[pn - 1])[j - 1]);
                    //}
                    //catch
                    //{
                    //    buf = new Complex((double)((int)(((ArrayList)Values[pn - 1])[j - 1])));
                    //}
                    //
                    // return buf;                    
        ...
        /// <summary>
        /// Access to the pn-th component of an n by one matrix (column vector)
        /// or one by n matrix (row vector).
        /// </summary>
        /// <param name="pn">One-based index.</param>
        /// <returns></returns>
        public virtual Complex this[int i]
        {
            ...
            {                                                    
                ...
                //    try
                //    {
                //        buf = (Complex)(((ArrayList)Values[0])[pn - 1]);
                //    }
                //    catch
                //    {
                //        buf = new Complex((double)((int)(((ArrayList)Values[0])[pn - 1])));
                //    }
                //else
                //    try
                //    {
                //        buf = (Complex)(((ArrayList)Values[pn - 1])[0]);
                //    }
                //    catch
                //    {
                //        buf = new Complex((double)((int)(((ArrayList)Values[pn - 1])[0])));
                //    }
