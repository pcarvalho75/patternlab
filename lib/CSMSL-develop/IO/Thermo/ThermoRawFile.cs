...
namespace CSMSL.IO.Thermo
{
    public class ThermoRawFile : MSDataFile
    {
        ...
        public override int GetParentSpectrumNumber(int spectrumNumber)
        {
            ...
            int msnOrderCurrent = GetMsnOrder(spectrumNumber);

            int parentScanNumber = spectrumNumber - 1;
            int msnOrderParent = GetMsnOrder(parentScanNumber);

            while (msnOrderParent >= msnOrderCurrent)
            {
                msnOrderParent = GetMsnOrder(--parentScanNumber);
            }
        ...
        public double[,] GetUnlabeledData(int spectrumNumber, bool useCentroid)
        {
